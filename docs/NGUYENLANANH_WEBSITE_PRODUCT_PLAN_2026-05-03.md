...
- Admin home priority summary is now actionable: each compact priority bucket can be clicked to apply the matching priority filter immediately, so operations can jump from overview to the exact queue slice in one step.
...
- Reflection ops and pilot ops now also show the imported priority mix from an intake queue packet, so operators can see whether the subset they opened leans toward reflection-now, missing-handoff, pilot-ready, paused, or already-routed cases without going back to admin home.
- Reflection ops and pilot ops now render a dedicated imported-subset summary block, so total queue size, relevant subset size, filter scope, and priority mix are visible as a compact module-local reference instead of living only inside the long status sentence.
- That imported-subset summary block now lets the operator reopen the same route, handoff, and priority filters on admin home, so the batch handoff loop can return to its source scope without reselecting filters by memory.
- Admin home now shows the active queue filter scope in plain text and offers one-click filter reset, so operators can immediately verify or clear a restored route, handoff, and priority scope after coming back from reflection or pilot ops.
- The active-filter banner on admin home now supports one-click removal of each individual filter dimension, so operators can drop only route, handoff, or priority without clearing the whole queue scope.
...
- Admin home now includes D1 queue sync controls (key input + pull/replace/push/clear actions), so operators can move intake queue packets between browser-local evidence and server-backed queue state without leaving the dashboard.
- Admin reflection and admin pilot now also include direct D1 queue load controls with handoff, priority, and item-limit filters, so operators can fetch the exact queue subset straight inside the destination module without returning to admin home first.
- When reflection or pilot loads a subset from D1, that subset is now merged into admin home browser queue storage too, so returning to admin home preserves the same operational packet context.
