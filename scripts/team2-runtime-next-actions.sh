...
vn_via_pay_iai_one="$(jq -r '.vn_via_pay_iai_one // 1' "$REPORT_PATH")"
...
echo "Phase flags: strict_mode=$strict_mode require_paypal=$require_paypal require_stripe=$require_stripe require_intl_provider=$require_intl_provider vn_via_pay_iai_one=$vn_via_pay_iai_one connectivity_failed=$connectivity_failed"
echo
...
if [ -n "$missing" ] || [ -n "$expected" ]; then
  ...
  echo "  TARGET_ENVS=\"production preview\" VN_VIA_PAY_IAI_ONE=$vn_via_pay_iai_one REQUIRE_PAYPAL=$require_paypal REQUIRE_STRIPE=$require_stripe bash scripts/provision-payment-live-secrets.sh"
  echo "- Redeploy:"
  echo "  bash scripts/deploy_cloudflare.sh"
fi
echo "- Re-run non-strict gate:"
echo "  BASE_URL=$base_url VN_VIA_PAY_IAI_ONE=$vn_via_pay_iai_one REQUIRE_PAYPAL=$require_paypal REQUIRE_STRIPE=$require_stripe REQUIRE_INTL_PROVIDER=$require_intl_provider STRICT_MODE=0 CHECK_PAGES_SECRETS=1 bash scripts/team2-runtime-phase-gate.sh"
echo "- Re-run strict gate:"
echo "  BASE_URL=$base_url VN_VIA_PAY_IAI_ONE=$vn_via_pay_iai_one REQUIRE_PAYPAL=$require_paypal REQUIRE_STRIPE=$require_stripe REQUIRE_INTL_PROVIDER=$require_intl_provider STRICT_MODE=1 CHECK_PAGES_SECRETS=1 bash scripts/team2-runtime-phase-gate.sh"
